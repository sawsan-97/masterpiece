...
<!-- قسم منتجاتنا المميزة -->
<section style="background-color: #ffffff; padding: 60px 0; font-family: 'Times New Roman', Times, serif;">
    <div class="container">
        <h2 style="color: #8B0000; font-size: 2rem; font-weight: 700; margin-bottom: 2rem; text-align: center;">منتجاتنا المميزة</h2>
        <div class="row">
    ...
    <div class="text-center">
        <a href="{{ route('products.index') }}" class="btn btn-primary" style="background-color: #a83232; border-color: #a83232;">عرض جميع المنتجات</a>
    </div>
</section>
        ...
        @forelse($news as $item)
            <div class="col-md-4 mb-4 d-flex">
                <div class="card h-100 shadow-sm" style="border-radius: 20px;">
                    @if($item->image)
                        <img src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->title }}" class="card-img-top" style="height: 250px; object-fit: cover; border-radius: 20px 20px 0 0;">
                    @endif
                    <div class="card-body d-flex flex-column text-center">
                        <h5 class="card-title" style="font-weight: bold;">{{ $item->title }}</h5>
                        <p class="card-text text-muted mb-3">{{ Str::limit(strip_tags($item->content), 80) }}</p>
                        <a href="{{ route('news.show', $item) }}" class="btn btn-link text-success mt-auto" style="font-weight: bold;">اقرأ المزيد</a>
                    </div>
                </div>
            </div>
        @empty
...
document.addEventListener('DOMContentLoaded', function() {
    // التأكد من ظهور رسائل النجاح والخطأ
